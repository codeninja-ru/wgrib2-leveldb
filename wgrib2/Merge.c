...
/*
 * HEADER:100:merge_fcst:output:2:normalize NCEP-type ave/acc X=number to intervals to merge Y=output grib file
 */
...
if (mode == 99)  fprintf(stderr,"merge_fcst: save->has_val=%d  ",save->has_val);
	...
	// only one level of ave/acc/min/max
...
//	    save end of date of end of overall time interval
