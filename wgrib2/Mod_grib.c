...
static const char *set_options="discipline, center, subcenter, master_table, local_table, background_process_id, "
	...
	"table_3.3, table_3.4, table_4.0/PDT, table_4.1, table_4.2, table_4.3, table_4.6, table_4.10, "
...
int f_set_metadata(ARG1) {
	...
	if ((len_arg1 = strlen(ftime))) {
	    if (strcmp(arg1,"anl") == 0) f_set_ftime(call_ARG1(inv_out,NULL,ftime));
	    else if ( ((i = sscanf(arg1,"%*d %*s %s%n", string, &len)) == 1) && len == len_arg1) f_set_ftime(call_ARG1(inv_out,NULL,ftime));
	...
	for (i = 0; i < n; i++) {
	    ...
	    if (j == 1 && strcmp(str1,"low-res ctl") == 0) {
		continue;
	    }
